...
def load_and_merge_data():
    """데이터 로드 및 기상 데이터 병합 (계절별 주산지 + Version 3 설정)"""
    # 1. 파일 로드
...
def plot_results(train_df, test_df, y_test, predictions, model_type):
    ...
    ax.set_title(f'2025년 가격 예측 (Ver.3 - 누적6순/시차0순) - {model_type}')
    ax.legend()
    ...
    plt.tight_layout()
    plt.savefig('price_prediction_v3_result.png')
    print("그래프 저장: price_prediction_v3_result.png")
...
def save_prediction_csv(test_df, y_test, predictions):
    ...
    result.to_csv('prediction_results_v3.csv', index=False, encoding='utf-8-sig')
    print("결과 저장: prediction_results_v3.csv")

def main():
    print("="*70)
    print("배추 가격 예측 모델 Ver.3 (누적 6순, 시차 0순)")
    print("="*70)
